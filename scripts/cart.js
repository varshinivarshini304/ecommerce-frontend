...
function renderCartItem(item, index) {
    const optionLines = item.options && Object.keys(item.options).length
        ? `<p class="cart-item-options">${Object.entries(item.options)
    ...
    return `
        <article class="cart-item" data-index="${index}">
            <div class="cart-item-image">
                <img src="${item.image || 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22150%22%20height%3D%22150%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Crect%20width%3D%22150%22%20height%3D%22150%22%20fill%3D%22%23f7f9fc%22/%3E%3C/svg%3E'}" alt="${item.title}" loading="lazy" />
            </div>
...
function renderCart() {
    ...
    if (!cart.length) {
        cartList.innerHTML = `
            <div class="cart-empty">
                ...
                <a class="primary-button" href="index.html">Continue shopping</a>
            </div>
        `;
    ...
    cartList.innerHTML = cart.map((item, index) => renderCartItem(item, index)).join('');
    cartTotalLabel.textContent = formatPrice(calculateCartTotal(cart));
...
function initializeCartActions() {
    cartList.addEventListener('click', (event) => {
        ...
        if (event.target.matches('.quantity-decrease')) {
            ...
            updateCartItem(index, Number(input.value) - 1);
        }
